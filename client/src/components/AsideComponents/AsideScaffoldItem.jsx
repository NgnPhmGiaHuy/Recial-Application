...
const AsideScaffoldItem = ({ userProps, navigationProps }) => {
    return (
        <li>
            <div>
                <a href={userProps.itemLink} className="block rounded-md hover:bg-zinc-100 transition-all">
                    <div className="min-h-[48px] px-[8px] flex flex-row items-center justify-between relative">
                        <div className="my-[6px] mr-[12px] flex flex-col self-center relative">
                            <div className={`${navigationProps === userProps.itemNavigation ? "bg-lime-500 text-white" : "bg-zinc-200"} w-[40px] h-[40px] flex flex-row items-center justify-center rounded-full overflow-hidden relative`}>
                                <i>{userProps.itemImage}</i>
                            </div>
                        </div>
