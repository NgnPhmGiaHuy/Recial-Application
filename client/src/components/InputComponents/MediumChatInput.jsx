const MediumChatInput = ({ forwardRef, inputText, handleInputState, handleShowIconPanel }) => {
    return (
        <div>
            <div className="flex justify-end relative">
                <div className="mx-[12px] my-[8px] flex flex-grow relative">
                    <div className="max-h-[124px] min-h-[20px] w-full h-full flex flex-grow items-center overflow-y-auto relative">
                        <div className="w-full h-full text-black select-text whitespace-pre-wrap break-words outline-none relative" contentEditable={true} spellCheck={true} tabIndex={0} onInput={handleInputState} ref={forwardRef}>
                        </div>
                        <div className="top-[0px] overflow-hidden text-zinc-500 text-ellipsis pointer-events-none absolute z-[1]">{inputText?.length === 0 ? `Aa` : null}
                        </div>
                    </div>
                </div>
                <div className="ml-[-12px] pb-[8px] pr-[4px] flex items-center justify-center self-end relative">
                    <div onClick={handleShowIconPanel}>
