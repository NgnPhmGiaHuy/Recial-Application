...
const MediaPageScaffoldHeaderReaction = () => {
    ...
    const reactionsUserId = mediaProps?.reaction?.map(reaction => reaction.user._id);
    const hasLikeReaction = reactionsUserId?.includes(userProps?.user?._id);

    const handleReaction = () => {
        if (hasLikeReaction) {
            return handleReactionData("Like", mediaProps?._id, "DELETE");
        }
        return handleReactionData("Like", mediaProps?._id);
    }
                ...
                <div className="flex flex-row items-center">
                    <div className="mr-[20px] flex flex-row items-center cursor-pointer">
                        <div onClick={handleReaction}>
                            <div className={`${hasLikeReaction ? "bg-red-100" : "bg-zinc-100 hover:bg-zinc-200"} w-[32px] h-[32px] flex flex-row items-center justify-center rounded-full overflow-hidden relative transition-all`}>
                                <i>
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`${hasLikeReaction ? "fill-red-500 border-none stroke-red-400" : "fill-none"} w-6 h-6`}>
                        ...
                        <div className="ml-[4px] flex items-center">
                            <span className="block text-[12px] text-left font-normal break-words relative leading-5">
                                <span className="overflow-hidden relative">
                                    { totalLike ? (handleFormatNumber(totalLike)) : 0 }
                                </span>
                            </span>
                        </div>
                    </div>
