...
import {PostItemCommentScaffold, QuickSettingItem} from "@/components";
...
const PostItemComment = ({userProps, postProps}) => {
    ...
    const loadMoreComments = () => {
        const remainingComments = postProps.postComments.length - visibleComments;
        const loadMore = remainingComments > 3 ? 3 : remainingComments;
        setVisibleComments((prevVisible) => prevVisible + loadMore);
        if (visibleComments + loadMore >= postProps.postComments.length) {
    ...
    return (
        <div className="flex flex-col justify-center relative">
                    ...
                    <Link href={userProps?.user?._id}>
                        <div className="w-[40px] h-[40px] overflow-hidden rounded-full relative">
                            <Image src={userProps?.user?.profile_picture_url}
                                   alt={`${userProps?.user?.profile_picture_url}-image`} fill={true}
                    ...
                    <form action="" className="w-full h-full grow cursor-text relative">
                        <div className="w-full h-full px-[16px] py-[2px] rounded-3xl border-2 border-solid border-zinc-200 bg-white relative">
                            <div className="w-full h-full flex flex-wrap items-center relative">
                                <div
                                    className="w-auto h-full pb-[8px] pt-[6px] text-[14px] text-black text-ellipsis flex flex-auto overflow-x-hidden overflow-y-hidden font-normal leading-5">
                                    <div
                                        className="w-full h-full text-left outline-none whitespace-pre-wrap break-words relative"
                                        contentEditable={true} onInput={handleCommentInputTextChange}
                                        ref={commentInputContentEditableRef}>
                                    </div>
                                    <div
                                        className="top-[7px] overflow-x-hidden overflow-y-hidden text-zinc-500 text-ellipsis pointer-events-none absolute z-[1]">
                                ...
                                <div className="h-full ml-auto flex flex-row items-center justify-between relative">
                                    <div
                                        className="w-[44px] h-[36px] flex flex-col items-center justify-center text-zinc-500 rounded-full cursor-pointer relative hover:bg-zinc-200 transition-all">
                                        <i>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                                 viewBox="0 0 24 24" strokeWidth={1.5} stroke="none"
                                                 className="w-7 h-7">
                                                <path
                                                    d="M8 10.5A1.5 1.5 0 119.5 12 1.5 1.5 0 018 10.5zm6.5 1.5a1.5 1.5 0 10-1.5-1.5 1.5 1.5 0 001.5 1.5zm7.5 0A10 10 0 1112 2a10 10 0 0110 10zm-2 0a8 8 0 10-8 8 8 8 0 008-8zm-8 4a6 6 0 01-4.24-1.76l-.71.76a7 7 0 009.89 0l-.71-.71A6 6 0 0112 16z"></path>
                                            </svg>
                                        </i>
                                    </div>
                                    <div
            ...
            <div>
                {postProps.postComments.slice(0, visibleComments).map((value, index) => (
                    <PostItemCommentScaffold key={index} postProps={value} />
                ))}
            </div>
            {!loadMoreClicked && visibleComments < postProps.postComments.length && (
                <div className="mx-[16px] mb-[8px] relative">
