...
const Story = ({userData}) => {
    ...
    const handleShowNextStory = () => {
        flushSync(() => {
            ...
            if (newIndex < userData.stories.length) {
                setStoryItemIndex(newIndex);
            } else {
                setStoryItemIndex(userData.stories.length - 1);
            }
        });
        if (storyItemSelectedRef.current) {
            storyItemSelectedRef.current.scrollIntoView({
            ...
            <div className="min-h-[200px] mb-[14px] relative">
                {storyItemIndex === 0 ? "" : (
                    <div className="top-[50%] left-[44px] translate-x-[calc(-50%-4px)] -translate-y-1/2 absolute opacity-100 transition-opacity ease-in-out duration-300 z-20">
                        <div onClick={() => handleShowPrevStory()} className="w-[48px] h-[48px] flex items-center justify-center shadow-md rounded-full cursor-pointer bg-white hover:bg-zinc-50 relative">
                    ...
                    <div className="py-[8px] flex flex-col overflow-x-auto overflow-y-hidden relative custom-story-scrollbar">
                        <div className="flex flex-row grow relative">
                            <div className="mr-[12px] flex flex-shrink-0 grow-0 basis-[160px] relative">
                                <a href="" className="w-full block overflow-x-hidden overflow-y-hidden cursor-pointer rounded-xl relative">
                                    <div className="w-full h-0 pt-[230px] overflow-x-hidden overflow-y-hidden shadow-md rounded-md relative">
                                        <div className="absolute top-0 right-0 bottom-0 left-0">
                                            <div className="h-full flex flex-col relative">
                                                <div className="h-full overflow-x-hidden overflow-y-hidden relative transition ease-in-out duration-150">
                                                    <Image src={userData.user.profile_picture_url} alt={`${userData.user.profile_picture_url}-image`} width={0} height={0} className="w-full h-full object-cover"/>
                                                </div>
                            ...
                            {userData.stories.map((value, index) => (
                ...
                {storyItemIndex === userData.stories.length - 2 ? "" : (
                    <div className="top-[50%] right-[44px] translate-x-[calc(50%+4px)] -translate-y-1/2 absolute opacity-100 transition-opacity ease-in-out duration-300">
                        <div onClick={() => handleShowNextStory()} className="w-[48px] h-[48px] flex items-center justify-center shadow-md rounded-full cursor-pointer bg-white hover:bg-zinc-50 relative">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
