...
const UserProfileCover = ({userProps, navigationProps}) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (peopleYouMayKnowButtonRef.current && !peopleYouMayKnowButtonRef.current.contains(event.target)) {
                setShowPeopleUseMayKnow(false);
            }
        }
                    ...
                    <div className="top-0 right-0 bottom-0 left-0 absolute">
                        <figure className="w-full h-[134px] rounded-t-md overflow-hidden relative">
                            <div className="w-full h-full bg-white relative">
                                <Image src={userProps.user.profile_cover_photo_url} alt={`${userProps.user.profile_cover_photo_url}-image`} fill className="object-cover"/>
                            </div>
                        </figure>
                    </div>
                </div>
                                ...
                                <div className="flex flex-row items-center relative">
                                    ...
                                    <div className="flex flex-row items-center relative">
                                        ...
                                        <div className="pr-[4px] flex items-center before:mr-[4px] before:w-[2px] before:h-[2px] before:rounded-full before:bg-zinc-500">
                                            <span className="block text-[14px] text-zinc-500 text-left font-normal break-words leading-5">
                                                <span className="overflow-hidden relative">
                                                    {userProps.user.followers_count} follower
                                                </span>
                                            </span>
                                        </div>
                                        <div className="pr-[4px] flex items-center before:mr-[4px] before:w-[2px] before:h-[2px] before:rounded-full before:bg-zinc-500">
                                            <span className="block text-[14px] text-zinc-500 text-left font-normal break-words leading-5">
                                                <span className="overflow-hidden relative">
                                                    {userProps.user.following_count} following
                                                </span>
                                            </span>
                                        </div>
                                    </div>
