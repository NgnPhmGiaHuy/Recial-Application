    ...
    return (
        <>
            ...
            {mediaProps?.media?.media_recent?.length > 1 ? (
                <div>
                    <div className="h-full top-0 left-[10px] flex items-center justify-center cursor-pointer absolute z-50" onMouseEnter={handleShowPrevButton} onMouseLeave={handleShowPrevButton}>
                        <div className="w-[60px] h-[60px] relative">
                            { showPrevButton && <MediumPrevButton onClick={fetchPreviousMedia}/> }
                        </div>
                    </div>
                    <div className="h-full top-0 right-[10px] flex items-center justify-center cursor-pointer absolute z-50" onMouseEnter={handleShowNextButton} onMouseLeave={handleShowNextButton}>
                        <div className="w-[60px] h-[60px] relative">
                            { showNextButton && <MediumNextButton onClick={fetchNextMedia}/> }
                        </div>
                    </div>
                </div>
            ) : null}
