...
const VideoPageScaffoldHeader = ({videoProps}) => {
    const videoDescriptionRef = useRef(null);

    const [copyLinkSuccess, setCopyLinkSuccess] = useState('');
    const [isOverflowing, setIsOverflowing] = useState(false);
    const [showMoreAboutVideoDescription, setShowMoreAboutVideoDescription] = useState(false);

    const handleCopyToClipboard = useCallback(() => {
        const linkToCopy = `https://www.recial.com/video/${videoProps.video_id}`;
        navigator.clipboard.writeText(linkToCopy)
            .then(() => setCopyLinkSuccess('Copied!'))
            .catch((error) => console.error('Failed to copy:', error));
    }, [videoProps.video_id]);

    const handleShowMoreAboutVideoDescription = useCallback(() => {
        setShowMoreAboutVideoDescription((prevShowMoreAboutVideoDescription) => !prevShowMoreAboutVideoDescription);
    }, []);

    useEffect(() => {
        if (videoDescriptionRef.current) {
            const { clientHeight, scrollHeight } = videoDescriptionRef.current;
            setIsOverflowing(scrollHeight > clientHeight)
        }
    }, [videoProps.video_description]);
            ...
            <div className="mx-[20px] p-[16px] rounded-md bg-zinc-100">
                ...
                <div className="flex flex-col overflow-visible">
                    <div className="flex flex-col w-full overflow-hidden relative">
                        <div ref={videoDescriptionRef} className={`${showMoreAboutVideoDescription ? "" : "line-clamp-2 max-h-[44px]"} w-full text-ellipsis overflow-hidden relative before:h-[calc(100%-21px)] before:w-0 before:mt-0 before:float-right`}>
                            <span className="text-[16px] text-black text-left relative leading-5">
                                <h1>
                        ...
                        {!showMoreAboutVideoDescription && isOverflowing ? (
                            <div className="flex items-center rounded-md cursor-pointer bg-zinc-100 z-10">
                                <span className="text-[16px] text-zinc-500 text-left font-semibold break-words relative leading-5" onClick={handleShowMoreAboutVideoDescription}>
                                    <span className="overflow-hidden relative">
                                        More
                                    </span>
                                </span>
                    ...
                    <div className="flex flex-row items-center">
                        <div className="mr-[20px] flex flex-row items-center cursor-pointer">
                            ...
                            <div className="ml-[4px] flex items-center">
                                <span className="block text-[12px] text-left font-normal break-words relative leading-5">
                                    <span className="overflow-hidden relative">
                                        {handleFormatNumber(videoProps.likes_count)}
                                    </span>
                                </span>
                            </div>
                        </div>
                        <div className="mr-[20px] flex flex-row items-center cursor-pointer">
                            ...
                            <div className="ml-[4px] flex items-center">
                                <span className="block text-[12px] text-left font-normal break-words relative leading-5">
                                    <span className="overflow-hidden relative">
                                        {handleFormatNumber(videoProps.comments.length)}
                                    </span>
                                </span>
                            </div>
                        </div>
                        <div className="mr-[20px] flex flex-row items-center cursor-pointer">
                            ...
                            <div className="ml-[4px] flex items-center">
                                <span className="block text-[12px] text-left font-normal break-words relative leading-5">
                                    <span className="overflow-hidden relative">
                                        {handleFormatNumber(videoProps.shares_count)}
                                    </span>
                                </span>
                            </div>
                        </div>
                ...
                <div className="mt-[16px] flex flex-row rounded-md border border-solid border-zinc-200 bg-zinc-100 overflow-hidden relative">
                    <div className="w-full flex flex-row items-center text-[14px] text-zinc-500 text-left font-normal break-words relative overflow-hidden leading-5">
                        <p className="pt-[7px] pb-[5px] pl-[12px] flex-[1_1_auto] text-ellipsis whitespace-nowrap overflow-hidden relative">
                            https://www.recial.com/video/{videoProps.video_id}
                        </p>
...
export default VideoPageScaffoldHeader;